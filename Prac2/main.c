...
int main(void){
  ...
  while (1)
  {
	...
	getTime();
	//setTime(0,38,1,3,6,9,22);

	sprintf(buffer, "%d \r\n", epochFromTime(time));
	HAL_UART_Transmit(&huart2,(uint8_t*)buffer , sizeof(buffer), 1000);


	sprintf (buffer, "%02d:%02d:%02d\r", time.hour, time.minutes, time.seconds);
...
void pause_sec(float x)
{
	...
	//YOUR CODE HERE
	//The above code is used is accurate to within 10 milliseconds, and was made through trial and error
	//using the oscilloscope, due to the imperfect accuracy when timing for longer than 1 second this deviation
	//has a larger impact, but for the purposes of this practical it is fine.
}
...
uint8_t temp_day=time.dayofmonth;
uint8_t temp_month=time.month;
	switch(temp_month){
	case 2:
		temp_day += 31;
	break;
	...
	temp_day+=365*time.year;

	return EPOCH_2022+(temp_day*24*60*60)+time.hour*60*60+time.minutes*60+time.seconds;
}
