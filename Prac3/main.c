...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  sprintf(buffer, "%s\r\n", "-----");
	  HAL_UART_Transmit(&huart2,(uint8_t*)buffer , sizeof(buffer), 1000);// just making the output look nice
	  //TO DO:
	  //TASK 2
	  ...
	  HAL_Delay (freq*100); // wait for 500 ms
...
void EXTI0_1_IRQHandler(void)
{
	...
	//the problem that this code may face is to do with the user interacting with the button and
	//sometimes pressing the button for long enough that the method above is actually called more
	//than once and sometimes resulting in the frequency staying the same. to fix this problem we can
	//create a sort of deadtime where the code cannot be run so quickly after its last implementation,
	//using the HAL_GetTick() function we can fix this by waiting for 10miliseconds between possible runs.
